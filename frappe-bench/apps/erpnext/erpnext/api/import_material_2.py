...
def check_warehouse(warehouse_name, line_num, warehouses_temp):
    """Vérifie si l'entrepôt existe pour la compagnie actuelle, sinon le crée."""
    if not warehouse_name:
        frappe.throw(f"Ligne {line_num}: target_warehouse est vide.")
    
    company = frappe.db.get_single_value("Global Defaults", "default_company")
    if not company:
        frappe.throw(f"Ligne {line_num}: Aucune compagnie par défaut définie dans Global Defaults.")

    print(f'Le warehouse_name "{warehouse_name}"')
    print(f'La company "{company}"')
    ...
    if existing_warehouse:
        print(f"Entrepôt trouvé : {existing_warehouse}")
    ...
    print(f"Entrepôt '{warehouse_doc_name}' ajouté à warehouses_temp.")
    return warehouse_doc_name
